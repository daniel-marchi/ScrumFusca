...
namespace ScrumFusca
{
    public class InjecaoEletronica
    {
        ...
        public InjecaoEletronica(object bateria, object vela, object valvula) {
            if (bateria == null)
                throw new ArgumentNullException("O Módulo de Injeção requer uma bateria");
                
            if (vela == null)
                throw new ArgumentNullException("O Módulo de Injeção requer velas");

            if (valvula == null)
                throw new ArgumentNullException("O Módulo de Injeção requer valvulas");
